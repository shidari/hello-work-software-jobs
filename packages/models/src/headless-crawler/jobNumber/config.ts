...
export const partialWorkLocationSchema = v.object({ prefecture: v.literal("東京都") });
export const partialEmploymentTypeSchema = v.union([v.literal("RegularEmployee"), v.literal("PartTimeWorker")])
export const paritalEngineeringLabelSchema = v.literal("ソフトウェア開発技術者、プログラマー")
export const searchPeriodSchema = v.union([v.literal("all"), v.literal("today"), v.literal("week")])
export const jobSearchCriteriaSchema = v.object({
    jobNumber: v.optional(jobNumberSchema),
    workLocation: partialWorkLocationSchema,
    desiredOccupation: v.optional(v.object({
