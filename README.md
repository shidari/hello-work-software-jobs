...
開発の主軸は **Apple container ベースの dev サンドボックス** (`sho-sandbox`)。Claude Code 本体と汎用 dev tools (`nodejs` / `pnpm` / `deno` / `jq` / chromium) のみを抱える最小実行環境です。認証 token を抱える CLI (`gh` / `wrangler` / `vercel` / `awscli`) は **host (macOS) 専用** とし、sandbox には入れていません。実 AWS への参照は **ops サンドボックス** (`sho-mcp-ops`) に閉じた read-only MCP server（GitHub / AWS API / CloudWatch）経由。詳細は [.claude/rules/cli.md](.claude/rules/cli.md) 参照。
