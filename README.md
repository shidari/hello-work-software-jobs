...
- **診断**: `/crawler-diagnose` / `/debug` skill + ops MCP（GitHub / AWS API / CloudWatch）+ host の `wrangler` / `vercel` + sandbox の `jq`

## セットアップ

開発の主軸は **Apple container ベースの dev サンドボックス** (`sho-sandbox`)。Claude Code 本体と汎用 dev tools (`nodejs` / `pnpm` / `deno` / `jq` / chromium) のみを抱える最小実行環境です。認証 token を抱える CLI (`gh` / `wrangler` / `vercel` / `awscli`) は **host (macOS) 専用** とし、sandbox には入れていません。実 AWS への参照は **ops サンドボックス** (`sho-mcp-ops`) に閉じた MCP server（GitHub / AWS API / CloudWatch）経由。GitHub は PAT scope に従い write も可、AWS 系は read-only。詳細は [.claude/rules/cli.md](.claude/rules/cli.md) 参照。

```bash
# dev サンドボックス（初回 or flake.nix 更新時）
./scripts/sandbox-image.ts   # image build + smoke test + container 再作成
./scripts/sandbox.ts         # 以後はこれでコンテナに入る（direnv 入れていれば自動 up）

# ops サンドボックス（GitHub / AWS の MCP server を上げる、初回 or flake.nix 更新時）
./packages/mcp-ops/ops-sandbox-image.ts   # image build + container 再作成
./packages/mcp-ops/ops-sandbox.ts         # 通常起動（Keychain から GitHub PAT を取り直す）
