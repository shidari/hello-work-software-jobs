...
async function main() {
  const rawHtml = await readFile(resolve("lib/T-jobDetail/playground/sample.html"), "utf-8");
  const program = buildProgram(rawHtml)
  // いまいち書き方がわかってない
  const runnable = program.pipe(Effect.provide(transformerLive)).pipe(Effect.provide(ConfigLive))
