...
```
CLAUDE.md              # Claude Code エントリポイント（コマンド集 + Rules 一覧）
...
`.claude/rules/` 配下にパッケージごとのルールが置かれている（[CLAUDE.md](CLAUDE.md) の Rules セクションが正規の一覧）。各ファイルは独立した実体で、シンボリックリンクではない。
