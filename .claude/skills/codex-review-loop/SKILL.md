...
./.claude/skills/codex-review-loop/loop.sh 5      # 位置引数で周回数指定
MAX_ROUNDS=5 ./.claude/skills/codex-review-loop/loop.sh  # 環境変数でも可
```

周回数の決定は Claude (本セッション) に任されている。差分のサイズ・複雑度を見て:

- 小さい差分 (1〜2 ファイル、ロジック変更なし) → 1〜2 周
- 中規模 (複数ファイル、ロジックあり) → 3 周（デフォルト）
- 大きい差分 (リファクタ・新機能) → 5 周程度
...
3. 差分が増えなければ早期終了、そうでなければ次周へ。最大 `MAX_ROUNDS` 周（デフォルト 3）
...
- script 内の `claude -p` は `--dangerously-skip-permissions` で動く。サンドボックス境界に閉じている前提なので OK だが、ホストで直接走らせない
