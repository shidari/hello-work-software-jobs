...
- **sandbox 内での実行必須**。子 claude が `--dangerously-skip-permissions` で動くため、host 直接実行はファイルシステム / ネットワークに無制限アクセスを与えることになり危険。`loop.sh` 冒頭で `SHO_SANDBOX=1` を確認し、無ければ exit 1 する（[.claude/hooks/deny-host.sh](../../hooks/deny-host.sh) と同じ env を見る。deny-host 側は primary に kernel + bind-mount で判定）
